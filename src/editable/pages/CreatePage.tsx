          ...
          <section className="mx-auto grid max-w-5xl gap-8 rounded-lg border border-[var(--editable-border)] bg-white p-7 shadow-[0_24px_70px_rgba(0,0,0,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-full bg-[#5f35b2] px-6 py-3 text-sm font-black text-white">Login <ArrowRight className="h-4 w-4" /></Link>
          ...
          <div className="grid gap-8 rounded-lg border border-[var(--editable-border)] bg-white p-6 shadow-[0_24px_70px_rgba(0,0,0,0.08)] lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-lg border p-4 text-left transition ${active ? 'border-[#5f35b2] bg-[#5f35b2] text-white' : 'border-[var(--editable-border)] bg-white hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-lg border border-[var(--editable-border)] bg-[#f7f7fa] p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[#5f35b2]">{session.name}</span>
              </div>
              ...
              <div className="mt-6 grid gap-4">
                ...
                <textarea className={`${fieldClass} min-h-24`} value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="Short buyer-facing business summary" required />
                <textarea className={`${fieldClass} min-h-48`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Services, specialties, service areas, pricing notes, support details, and business description" required />
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#5f35b2] px-6 text-sm font-black uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5">
